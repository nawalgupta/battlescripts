...
bsapp.factory('$battlescripts', ["$firebaseArray", "$firebaseObject","$firebaseAuth","$rootScope","$timeout", function($firebaseArray,$firebaseObject,$firebaseAuth,$rootScope,$timeout) {
  ...
  api.get_dev_game = (id,user_id) => {
    var url = "users/"+(user_id||api.user.uid)+"/games";
    var p = $firebaseObject(firebase.database().ref(url).child(id)).$loaded().catch(()=>{});
    return p;
  };
  ...
  api.Player = function(code,options) {
	...
	var noop = function(){};
    this.start_game = function(data) { try { return (this.player.start_game||noop)(data); } catch(e){} };
    this.end_game = function(data) { try { return (this.player.end_game||noop)(data); } catch(e){} };
    this.start_match = function(data) { try { return (this.player.start_match||noop)(data); } catch(e){} };
    this.end_match = function(data) { try { return (this.player.end_match||noop)(data); } catch(e){} };
